...
from open_r1.trainer import Qwen2VLGRPOTrainer, GRPOConfig,Qwen2VLGRPOTrainer_v13_lora
...
mpmodel = SentenceTransformer('sentence-transformers/all-mpnet-base-v2')
def sentence_recall_reward(reference, summary, threshold=0.4):
        """
        Compute recall reward at sentence level
        
        Args:
            reference: Source text
            summary: Generated summary
            threshold: Similarity threshold for considering a sentence "covered"
            
        Returns:
            recall_reward: Float between 0 and 1
        """
        # Split into sentences
        ref_sentences = [s.strip() for s in reference.split('.') if s.strip()]
        sum_sentences = [s.strip() for s in summary.split('.') if s.strip()]
        
        if not ref_sentences or not sum_sentences:
            return 0.0
        
        # Encode sentences
        ref_embeddings = mpmodel.encode(
            ref_sentences, 
            convert_to_tensor=True,
            show_progress_bar=False
        )
        sum_embeddings = mpmodel.encode(
            sum_sentences,
            convert_to_tensor=True,
            show_progress_bar=False
        )
        ...
        cosine_scores = util.cos_sim(ref_embeddings, sum_embeddings)
        
        # For each reference sentence, find max similarity with summary sentences
        max_similarities = cosine_scores.max(dim=1).values
        
        # Recall: proportion of reference sentences covered
        covered = (max_similarities >= threshold).float()
        recall_reward = covered.mean().item()
...
class GRPOScriptArguments(ScriptArguments):
    ...
    dataset_decision: Optional[str] = field(
        default=None,
        metadata={"help": "YAML file path for the decision detection dataset"},
    )
...
PERCEPTION_SYSTEM_PROMPT = (
    ...
    "First describes the low-level visual contents of the image, the advantage of this image (what makes this image looks good) and the flaws (what makes the image looks bad). Second  thinks about the reasoning process in the mind and then provides the user with the answer. The  subject,advantage, flaw, reasoning "
    " and answer are enclosed within <subject></subject><advantage></advantage><flaw></flaw> , <think> </think> and <answer> </answer> tags, respectively, i.e., "
    "<subject> the main image subject here</subject><advantage> the advantage here</advantage><flaw> the flaw here</flaw>, <think> reasoning process here </think><answer> answer here </answer>."
)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if cot_solution_0 !=' ':
            try:
                cot_solution_0 = cot_solution_0.get("answer","")
                cot_solution_1 = cot_solution_1.get("answer","")
                cot_solution_2 = cot_solution_2.get("answer","")
        ...
        sample["cot_sub"] = cot_solution_0
        sample["cot_good"] = cot_solution_1
        sample["cot_bad"] = cot_solution_2
        return sample
...
def cot_reward(completions, cot_sub, cot_good,cot_bad,**kwargs):
    """
    Compute the reward based on the format and content of the generated answers.
    ...
    # Define regular expression patterns
    sub_pattern = r'<subject>(.*?)</subject>'
    advantage_pattern = r'<advantage>(.*?)</advantage>'
    flaw_pattern = r"<flaw>(.*?)</flaw>"
    score_pattern = r'\"rating\"\s*:\s*([\d\.]+)'
    #print(f"\033[91m NUM:solution,contents ~~{len(solution)} {len(contents)} \033[0m")
    # Compute the sampling ratio to align 'task' length with number of completions
    ...
    subsampled_cot_goods = cot_good[::num_gen]
    subsampled_cot_bads = cot_bad[::num_gen]

    
    for i, (content, cot_sub,cot_good,cot_bad) in enumerate(zip(contents, subsampled_cot_subs, subsampled_cot_goods,subsampled_cot_bads)):
        reward = 0.0
        if content=="holder" or is_all_whitespace(cot_sub):
            reward = 0.0
        else:
            try:
                # Extract the answer content from the <answer> tag
                subject_answer = re.search(sub_pattern, content, re.DOTALL)
                advantage_answer = re.search(advantage_pattern, content, re.DOTALL)
                flaw_answer = re.search(flaw_pattern,content,re.DOTALL)
                if subject_answer:
                    subject_answer = subject_answer.group(1).strip()
                    if subject_answer:
                        reward+= sentence_recall_reward(subject_answer,cot_sub)/6

                if advantage_answer:
                    advantage_answer = advantage_answer.group(1).strip()
                    if advantage_answer:
                        reward+= sentence_recall_reward(advantage_answer,cot_good)/6
                
                if flaw_answer:
                    flaw_answer = flaw_answer.group(1).strip()
                    if flaw_answer:
                        reward+= sentence_recall_reward(advantage_answer,cot_good)/6
            except Exception as e:
                print("Error in computing reward", e)
...
def main(script_args, training_args, model_args):
    ...
    trainer_cls = Qwen2VLGRPOTrainer_v13_lora
