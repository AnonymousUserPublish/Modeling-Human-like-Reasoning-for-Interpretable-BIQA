...
from open_r1.trainer import Qwen2VLGRPOTrainer, GRPOConfig,Qwen2VLGRPOTrainer_lora
...
def sentence_recall_reward(reference, summary, threshold=0.4):
        ...
        recall_reward  = max_similarities.mean().item()
...
PERCEPTION_SYSTEM_PROMPT = (
    ...
    " Thinks about the reasoning process in the mind and then provides the user with the answer. The description, reasoningand answer are enclosed within <caption></caption> <think> </think> and <answer> </answer> tags, respectively, i.e., "
    "<caption>description here</caption><think> reasoning process here </think><answer> answer here </answer>. Do not mention any rating in caption or think sections."
)
...
def smooth_function(x):
    return 0.5*0.5 * (1 + np.cos(np.pi * x / 0.35))
    ...
    for i, (content, cot_sub) in enumerate(zip(contents, subsampled_cot_subs)):
        ...
        if content=="holder" or is_all_whitespace(cot_sub):
            ...
            try:
                ...
                lexical_scores = scorer.score(cot_sub.lower(), subject_answer.lower())
                rouge1_score =   lexical_scores['rouge1']
                precision = rouge1_score.precision
                cot_reward_lexical_semantic = 0.5*precision+0.5*sentence_recall_reward(cot_sub,subject_answer)/6
                reward+= cot_reward_lexical_semantic 
...
def format_reward(completions, **kwargs):
    ...
    return [0.3 if match is not None else 0.0 for match in matches]
...
def main(script_args, training_args, model_args):
    ...
    trainer_cls = Qwen2VLGRPOTrainer_lora
